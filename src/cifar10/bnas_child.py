...
class MicroChild(Model):
  def __init__(self,
    ...
    self.pool_distances = self.num_layers // (self.num_pool) - 1
    self.pool_layers = []
    if self.num_layers == self.num_pool:
      for pool_id in range(self.num_pool):
        self.pool_layers.append(pool_id)
      self.conv_list = self.pool_layers
    else:
      self.pool_layers = [self.pool_distances, 2 * self.pool_distances + 1]
      self.conv_list = [self.pool_distances - 1, 2 * self.pool_distances]

    print(self.pool_layers)
    print(self.conv_list)
    # for pool_id in range(self.num_pool):
    #   self.pool_layers.append((pool_id + 1) * self.pool_distances)
  ...
  def _maybe_calibrate_size(self, layers, out_filters, normal_or_reduction_cell, is_training):
    ...
    with tf.variable_scope("{0}_calibrate".format(normal_or_reduction_cell)):
      x = layers[0]
      if hw[0] != hw[1]:
        assert hw[0] == 2 * hw[1]
  ...
  def _model(self, images, is_training, reuse=False):
    ...
    with tf.variable_scope(self.name, reuse=reuse):
      ...
      with tf.variable_scope("stem_conv"):
        w = create_weight("w", [3, 3, 3, self.out_filters * 2])
        x = tf.nn.conv2d(
      ...
      if self.fixed_arc is None and self.num_layers == self.num_pool:
        c_list.append(out_filters//2)
      else:
        c_list.append(out_filters)
      for layer_id in range(self.num_layers):
        with tf.variable_scope("layer_{0}".format(layer_id)):
          if layer_id not in self.pool_layers:
            if self.fixed_arc is None:
              x = self._enas_layer(
                layer_id, layers, out_filters, normal_or_reduction_cell="normal")
              layers = [layers[-1], x]
              if layer_id in self.conv_list:
                conv_layers.append(x)
                for i in range(len(conv_layers)):
                  print(conv_layers[i])
                if len(conv_layers) > 1:
                  for i in range(len(conv_layers)-1):
                    with tf.variable_scope("conv_layers_{0}".format(i)):
                      conv_layers[i] = self._factorized_reduction(
                        conv_layers[i], c_list[i], 2, is_training)

            else:
              x = self._fixed_layer(
                layer_id, layers, out_filters, 1, is_training, normal_or_reduction_cell="normal")
              layers = [layers[-1], x]
              if layer_id in self.conv_list:
                conv_layers.append(x)
                if len(conv_layers) > 1:
            ...
            if self.fixed_arc is None:
              if self.num_layers == self.num_pool:
                ...
                if len(conv_layers) > 1:
                  for i in range(len(conv_layers)-1):
                    with tf.variable_scope("skip_connection_{0}".format(i)):
                      conv_layers[i] = self._factorized_reduction(
                        conv_layers[i], c_list[i], 2, is_training)
              x = self._factorized_reduction(x, out_filters, 2, is_training)
              layers = [layers[-1], x]
              ...
              if layer_id == self.pool_layers[-1]:
                if self.num_layers != self.pool_layers:
                  for i in range(len(conv_layers)):
                    with tf.variable_scope("conv_layers_{0}".format(i)):
                      conv_layers[i] = self._factorized_reduction(
                        conv_layers[i], c_list[i], 2, is_training)
                for i in range(self.num_enhance):
                  with tf.variable_scope("enhangce_layers_{0}".format(i)):
                    print("Add enhangce_node_{} in Layer_{}".format(i, layer_id))
                    x = self._enas_layer(
                      layer_id, layers, out_filters, normal_or_reduction_cell="reduce")
                    enhangced_layers.append(x)
            else:
              x = self._fixed_layer(layer_id, layers, out_filters,
                                    2, is_training, normal_or_reduction_cell="normal")
              if layer_id in self.conv_list:
                conv_layers.append(x)
                if len(conv_layers) > 1:
                  for i in range(len(conv_layers)-1):
              ...
              if layer_id == self.pool_layers[-1]:
                ...
                for i in range(self.num_enhance):
                  with tf.variable_scope("enhangce_layers_{0}".format(i)):
                    print("Add enhangce_node_{} in Layer_{}".format(i, layer_id))
                    x = self._fixed_layer(layer_id, layers, out_filters, 1,
                                        is_training, normal_or_reduction_cell="reduce")
                    enhangced_layers.append(x)

      final_layers = []
      for i in range(len(conv_layers)):
        final_layers.append(conv_layers[i])
      for i in range(len(enhangced_layers)):
        final_layers.append((enhangced_layers[i]))
      for i in range(len(final_layers)):
        print(final_layers[i])

      x = tf.concat(final_layers, axis=concate_axis)
      x_oc = self._get_C(x)
      print("The number of channels for GAP layer are {0}".format(x_oc))
      x = tf.nn.relu(x)
  ...
  def _fixed_layer(self, layer_id, prev_layers, out_filters, stride,
    ...
    """
    if normal_or_reduction_cell == "normal":
      arc = self.normal_arc
    elif normal_or_reduction_cell == "reduce":
      arc = self.reduce_arc
    else:
      raise ValueError("Unknown arc_type '{0}'".format(normal_or_reduction_cell))

    multiplier = 0

    assert len(prev_layers) == 2
    layers = [prev_layers[0], prev_layers[1]]
    layers = self._maybe_calibrate_size(layers, out_filters, normal_or_reduction_cell,
                                        is_training=is_training)

    with tf.variable_scope("{0}_layer_base".format(normal_or_reduction_cell)):
      x = layers[1]
      inp_c = self._get_C(x)
      w = create_weight("w", [1, 1, inp_c, out_filters])
      x = tf.nn.relu(x)
    ...
    for cell_id in range(self.num_cells):
      with tf.variable_scope("{0}_cell_{1}".format(normal_or_reduction_cell, cell_id)):
        x_id = arc[4 * cell_id]
        used[x_id] += 1
    ...
    print("Layer {0:>2d}: {1} {2}".format(layer_id, normal_or_reduction_cell, out))
    return out
  ...
  def _enas_layer(self, layer_id, prev_layers, out_filters,
                  normal_or_reduction_cell="normal"):
    """
    Args:
    ...
    layers = [prev_layers[0], prev_layers[1]]
    layers = self._maybe_calibrate_size(layers, out_filters, normal_or_reduction_cell, is_training=True)
    used = []
    for cell_id in range(self.num_cells):
    ...
    with tf.variable_scope("{0}_final_conv".format(normal_or_reduction_cell)):
